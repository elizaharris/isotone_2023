...
# get the Bai model
gaspart = pd.read_csv("data/Bai2012_Files/Other/para_Bai_WFPS.csv",delimiter=",") 

# import extended gas partitioning data
data = pd.read_csv("data/parameterisations_N_gases.csv",delimiter=",") 
# columns 5 6 7 =  WFPS(%)	 N2O/(N2O+NO)	 N2O/(N2O+N2)

# fit to the N2O/(N2O+NO) data
from scipy.optimize import curve_fit
xdata = data[' WFPS(%)'].astype("float")
ydata = data[' N2O/(N2O+NO)'].astype("float")
xdata = xdata[~ydata.isna()]
ydata = ydata[~ydata.isna()]
...
xdata = data[' WFPS(%)'].astype("float")
ydata = data[' N2O/(N2O+N2)'].astype("float")
xdata = xdata[~ydata.isna()]
ydata = ydata[~ydata.isna()]
...
# Function to find the fit to N2O/(N2+NO+N2O)
...
# Derivation of final parameterisation = N2O/(N2+NO+N2O): 
...
# Plot the final parameterisation
fig,ax = plt.subplots(3,1)
ax[0].plot(data[' WFPS(%)'],data[' N2O/(N2O+NO)'],"bo")
ax[0].plot(np.arange(0,100),yfit1,"b-")
ax[0].set_ylabel("N2O/(N2O+NO)")
ax[1].plot(data[' WFPS(%)'],data[' N2O/(N2O+N2)'],"bo")
...
ax[2].plot(gaspart["WFPS"]*100,gaspart["%N2O"],"r:") # Add the parameterisations from Bai et al. also
ax[2].plot(gaspart["WFPS"]*100,gaspart["%NO"],"g:")
ax[2].plot(gaspart["WFPS"]*100,gaspart["%N2"],"c:")
plt.legend(("N2O/(N2O+N2+NO)","NO/(N2O+N2+NO)","N2/(N2O+N2+NO)","Bai: N2O/(N2O+N2+NO)","Bai: NO/(N2O+N2+NO)","Bai: N2/(N2O+N2+NO)"),fontsize=5)
fig.tight_layout()
plt.savefig("figs/paramaterisations/N-gas-partitioning.png") 
plt.savefig("figs/paramaterisations/N-gas-partitioning.pdf") 
fig.show() 

# Create function to estimate partitioning based on WFPS
def gp_gaspart(WFPS,scale_fitNO = (1,1,1,1),scale_fitN2 = (1,1,1,1),plotYN="N"): # function to partition N2O, NO, N2 over the sum of the three
    ...
    if plotYN=="Y": # plot to check impact of scaling factors if needed
        ...
        plt.legend(("N2O/(N2O+N2+NO)","NO/(N2O+N2+NO)","N2/(N2O+N2+NO)","Orig: N2O/(N2O+N2+NO)","Orig: NO/(N2O+N2+NO)","Orig: N2/(N2O+N2+NO)"),fontsize=5)
        fig.tight_layout()
        plt.savefig("figs/paramaterisations/N-gas-partitioning_scalefactors.png") 
        plt.savefig("figs/paramaterisations/N-gas-partitioning_scalefactors.pdf") 
        fig.show() 
    return tmp
...
# import extended gas partitioning data
dataND = pd.read_csv("data/parameterisations_N2O_nit_denit.csv",delimiter=",") 
# 4 5 6 =  WFPS(%)	 f_nit	 f_denit

# fit to the data
xdata = dataND[' WFPS(%)'].astype("float")
ydata = dataND['f_N2O_nit'].astype("float")
xdata = xdata[~ydata.isna()]
ydata = ydata[~ydata.isna()]
p0 = [np.nanmax(ydata),np.median(xdata),0.1,np.nanmin(ydata)] # mandatory initial guess
...
ax[0].plot(dataND[' WFPS(%)'],dataND['f_N2O_denit'],"ro")
...
ax[2].plot(np.arange(0,100),res[:,2],"c-")
ax[2].set_ylim(0,1)
plt.legend(("N2Od/(N2O+N2+NO)","N2On/(N2O+N2+NO)","N2O/(N2O+N2+NO)","NO/(N2O+N2+NO)","N2/(N2O+N2+NO)"),fontsize=5)
fig.tight_layout()
plt.savefig("figs/paramaterisations/nit-denit-partitioning.png") 
plt.savefig("figs/paramaterisations/nit-denit-partitioning.pdf") 
fig.show()
...
def gp_nitdenit(WFPS,scale_fitnit = (1,1,1,1),plotYN="N"): # function to partition N2O, NO, N2 over the sum of the three
    ...
    if plotYN=="Y": # plot to check impact of scaling factors if needed
        ...
        plt.legend(("N2On/(N2O+N2+NO)","N2Od/(N2O+N2+NO)","Orig: N2On/(N2O+N2+NO)","Orig: N2Od/(N2O+N2+NO)"),fontsize=5)
        fig.tight_layout()
        plt.savefig("figs/paramaterisations/nit-denit-partitioning_scalefactors.png") 
        plt.savefig("figs/paramaterisations/nit-denit-partitioning_scalefactors.pdf") 
        fig.show()
    return tmp

# check scale changing
gp_nitdenit(np.arange(0,100),scale_fitnit = (0.5,1,1,1),plotYN="Y")

#%% Functions to plot final figurea
...
def gp_finalfig(scale_fitNO = (1,1,1,1),scale_fitN2 = (1,1,1,1)): # function to partition N2O, NO, N2 over the sum of the three
    ...
    plt.legend(("N2O/(N2O+N2+NO)","NO/(N2O+N2+NO)","N2/(N2O+N2+NO)",
                "Prior: N2O/(N2O+N2+NO)","Prior: NO/(N2O+N2+NO)","Prior: N2/(N2O+N2+NO)",
                "Bai: N2O/(N2O+N2+NO)","Bai: NO/(N2O+N2+NO)","Bai: N2/(N2O+N2+NO)" ),fontsize=5)
    ax[1].set_ylabel("N2O/(N2O+NO)")
    ax[1].plot(data[' WFPS(%)'],data[' N2O/(N2O+NO)'],"bo") 
    ax[1].plot(np.arange(0,100),yfit1,"b:")
    ax[1].plot(np.arange(0,100),res[:,0]/(res[:,0]+res[:,1]),"b")
    ax[2].set_ylabel("N2O/(N2O+N2)")
    ax[2].plot(data[' WFPS(%)'],data[' N2O/(N2O+N2)'],"bo") 
    ax[2].plot(np.arange(0,100),yfit2,"b:")
    ax[2].plot(np.arange(0,100),res[:,0]/(res[:,0]+res[:,2]),"b")
    fig.tight_layout()
    plt.savefig("figs/paramaterisations/N-gas-partitioning_final.png") 
    plt.savefig("figs/paramaterisations/N-gas-partitioning_final.pdf") 
    fig.show()
...
def gp_finalfig_error(scale_fitNO = (1,1,1,1),scale_fitN2 = (1,1,1,1),err_NO=(0,0,0,0),err_N2=(0,0,0,0)): # function to partition N2O, NO, N2 over the sum of the three
    ...
    fig.tight_layout()
    plt.savefig("figs/paramaterisations/nit-denit-partitioning_final.png") 
    plt.savefig("figs/paramaterisations/nit-denit-partitioning_final.pdf") 
    fig.show()
