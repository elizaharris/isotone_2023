...
if 0:
    ...
    params[0,:] = (preamble.c_prea[0], preamble.c_prea[1], preamble.c_prea[1], 1) # preanth N2O conc
    ...
    params[4,:] = (preamble.temp_sens,0.04,0.04,1) # temperature sensitivity of emissions
    params[5,:] = (preamble.ltPD_ltPI[0],preamble.ltPD_ltPI[1],preamble.ltPD_ltPI[1],1) # ratio of preindustrial to present day lifetime
    params[6,:] = (preamble.ltPD[0],preamble.ltPD[1],preamble.ltPD[1],1) # present day lifetime
    params[7,:] = (preamble.d15_ocean[0],preamble.d15_ocean[1],preamble.d15_ocean[1],1) # ocean source iso
    params[8,:] = (preamble.SP_ocean[0],preamble.SP_ocean[1],preamble.SP_ocean[1],1) # ocean source iso
    params[9,:] = (preamble.d15_prea[0],preamble.d15_prea[1],preamble.d15_prea[1],1)  # preI trop iso composition
    params[10,:] = (preamble.scale_fitNO[1],0.7,1.3,0) # NO emission scaling factor: restictions keep it in the range of obs
    params[11,:] = (preamble.SP_prea[0],preamble.SP_prea[1],preamble.SP_prea[1],1) # preI trop iso composition
    x = params[:,0].copy() # These are the input params for the model

#%% Define the function

def model(x, preamble, fullres="N",d15Nrandomise="N"):
    ...
    frac_expression = x[2]
    E_nit_new = [preamble.E_nit[0]*frac_expression,preamble.E_nit[1]]
    E_denit_new = [preamble.E_denit[0]*frac_expression,preamble.E_denit[1]]
    E_denit_N2O_new = [preamble.E_denit_N2O[0]*frac_expression,preamble.E_denit_N2O[1]]
    E_red_new = [preamble.E_red[0]*frac_expression,preamble.E_red[1]]
    E_SP_nit_new = [preamble.E_SP_nit[0]*frac_expression,preamble.E_SP_nit[1]]
    E_SP_denit_new = [preamble.E_SP_denit[0]*frac_expression,preamble.E_SP_denit[1]]
    E_SP_red_new = [preamble.E_SP_red[0]*frac_expression,preamble.E_SP_red[1]]
    ...
    d15N_vec = preamble.d15N_grid[datarng]
    # if d15N randomise option is selected, randomise using gridded uncertainty
    if d15Nrandomise!="N":
        tmp = np.random.randn(len(d15N_vec))*d15Nrandomise
        d15N_vec = d15N_vec + tmp*preamble.d15Nerr_grid[datarng]
        
    # get the partitioning for each d15N point; rows = d15N points; cols = N2O_all, N2_all, NO_all, denit_N2O, nit_N2O, red_N2O
    WFPS_vec = preamble.WFPS_grid[datarng]
    gaspart = preamble.para.gp_gaspart(WFPS_vec,scale_fitNO = scale_fitNO_new,scale_fitN2 = scale_fitN2_new,plotYN="N") # dict of N2O_all, N2_all, NO_all
    N2Opart = preamble.para.gp_nitdenit(WFPS_vec,scale_fitnit = preamble.scale_fitnit,plotYN="N") # dict of denit_N2O, nit_N2O
    ...
    fNH3_vec = preamble.fNH3_grid[datarng] # f of N as NH3 for each point
    ...
    tmp = isomodel.IsoModel(d15Nsoil_vec=d15N_vec,
                            ...
                            fNH3_vec=fNH3_vec,
                            fignum=np.nan,
                            d15N_inat=preamble.d15N_inat,
                            E_NH3=preamble.E_NH3,
                            E_L=preamble.E_L,
                            E_nit=E_nit_new,
                            E_denit=E_denit_new,
                            ...
                            f_soilpoolloss=preamble.f_soilpoolloss)
    ...
    k_L = np.zeros(preamble.d15N_grid.shape)+np.nan; k_L[datarng] = tmp[:,0]
    k_NH3 = np.zeros(preamble.d15N_grid.shape)+np.nan; k_NH3[datarng] = tmp[:,1]
    k_G = np.zeros(preamble.d15N_grid.shape)+np.nan; k_G[datarng] = tmp[:,2]
    f_denit = np.zeros(preamble.d15N_grid.shape)+np.nan; f_denit[datarng] = N_processes_vec[:,3]
    f_N2O = np.zeros(preamble.d15N_grid.shape)+np.nan; f_N2O[datarng] = N_processes_vec[:,0]
    d15N_modoutput = np.zeros(preamble.d15N_grid.shape)+np.nan; d15N_modoutput[datarng] = tmp[:,3]
    N2O_d15N = np.zeros(preamble.d15N_grid.shape)+np.nan; N2O_d15N[datarng] = tmp[:,4]
    N2O_SP = np.zeros(preamble.d15N_grid.shape)+np.nan; N2O_SP[datarng] = tmp[:,5]
    soilmodres = tmp
    ...
    Nsummary = np.zeros((preamble.years.shape[0],10)) 
    Nsummary[:,0] = preamble.years
    Nsummary_full = np.zeros((preamble.years.shape[0],23)) 
    Nsummary_full[:,0] = preamble.years
    
    # Unit/area conversion factor for emissions
    EF_N2O = (k_G*f_N2O)[datarng]  # Emission factor of N2O
    factor_area = (preamble.area_grid/1000/1000/1000/1000)[datarng] # converts from g m-2 y-1 to Tg y-1 per gridcell
    
    # find the years to use - nearest covered by the N inputs dataset
    datayears = preamble.years.copy()
    datayears[preamble.years<min(preamble.datarange)] = min(preamble.datarange)
    datayears[preamble.years>max(preamble.datarange)] = max(preamble.datarange)

    # cycle through the years to find integrated fluxes etc. for each year
    for n in range(0,preamble.years.shape[0]) :
        # get N input values for year n for fert, dep, fix
        index = int(np.where(preamble.datarange-datayears[n]==0)[0]) # find the "row" of the N input data for this year (use closest if that year not available)
        fix = preamble.Ninputs.variables["fixation"][index,:,:].data 
        fix = fix[datarng]
        dep =preamble. Ninputs.variables["deposition"][index,:,:].data
        dep = dep[datarng]
        fert = preamble.Ninputs.variables["fertilisation"][index,:,:].data
        fert = fert[datarng] 
        ...
        tindex = int(np.where(abs(preamble.Tanom.variables["time"]-preamble.years[n]) == np.min(abs(preamble.Tanom.variables["time"]-preamble.years[n])))[0])
        tanomaly = preamble.Tnorm[tindex,:,:] # T anomaly normalised to first year
        ...
        index = int(np.where(preamble.f.variables["time"][:]-datayears[n]==0)[0]) # find the "row" of the N input data for this year
        TRO = preamble.TRO_grid[index,:,:][datarng]
        WWT = preamble.WWT_grid[index,:,:][datarng]
        CHE = preamble.CHE_grid[index,:,:][datarng]
        ENE = preamble.ENE_grid[index,:,:][datarng]
    ...
    res_conc = np.interp(preamble.tstarts,results[:,0],results[:,2])
    res_d15 = np.interp(preamble.tstarts,results[:,0],results_d15[:,0])
    res_SP = np.interp(preamble.tstarts,results[:,0],results_SP[:,0])
...
def expand_modres(tmp, preamble):
    k_G = np.zeros(preamble.d15N_grid.shape)+np.nan; k_G[preamble.datarng] = tmp[3][:,2]
    f_N2O = np.zeros(preamble.d15N_grid.shape)+np.nan; f_N2O[preamble.datarng] = tmp[4][:,0]
    EF = k_G*f_N2O*100
    ### Check this is exactly the same split as in Final_Preamble for observations!
    EF_zones = preamble.climzone_means(var1_grid = preamble.MAT_grid, var2_grid = preamble.MAP_grid, datavar1 = preamble.MAT_grid,
                                  datavar2 = preamble.MAP_grid,data = EF,bins=4,plotfigs="N",LON=preamble.LON,LAT=preamble.LAT)[:,4]
