...
def atmos_model(Nsummary,
                MW_air = 28.9647,
                MW_N2O = 44,
                T_S = np.array([5.37e17,1.26e17]),
                m_trop = 0.85*1.77e20,
                m_strat = 0.15*1.77e20,
                ltPI = [123,10],
                ltPD = [116,9],
                c_prea = [250,7.5],
                d15_prea = [8.9,2.0],
    ...
    d15_terr = Nsummary[0,5] # preindustrial isotopic composition
    SP_terr = Nsummary[0,6] 
    d15_anth = Nsummary[:,5] # emitted isotopic composition
    SP_anth = Nsummary[:,6] 
    ...
    lt_t[t<=t0,0] = ltPI[0] # PI: all years before anthropocene
    slope = (ltPD[0] - ltPI[0])/(2010-t0)
    lt_t[t>t0,0] = ltPI[0] + (t[t>t0]-t0)*slope
    ...
    cstrat_prea_orig = cstrat_prea[0]

    # use soil module to estimate pre-I terrestrial flux
    F_terr_SM = Nsummary[0,1] 
    # assume steady state to find F_terr using eq. 2
    F_terr = - F_ocean[0] - T_Sm[0]*(cstrat_prea_orig - c_prea[0])*1e-9*28/1e12 
    diff = F_terr_SM - F_terr
    scale = diff/4 if diff/4 < 1 else 1
    F_ocean_new = F_ocean[0] - F_ocean[1]*scale
    T_Sm_new = T_Sm[0] + (T_S[1]*scale)*1000/MW_air
    F_terr = - F_ocean_new - T_Sm_new*(cstrat_prea_orig - c_prea[0])*1e-9*28/1e12
    # iterate other params to best match terr flux estimates
    n_max = 20; n = 0
    match_crit = 0.0001
    cstrat_prea_new = cstrat_prea_orig
    while abs(F_terr_SM - F_terr) > match_crit:
        diff = F_terr_SM - F_terr; 
    ...
    if isinstance(cstrat_prea_new, float): # For some reason, this is nearly always a float in an array, but sometimes just a float...
        cstrat_prea = cstrat_prea_new
    else:
        cstrat_prea = cstrat_prea_new[0]
    ...
    def preanth_del(del_ocean,del_prea,del_terr) :
        ...
        res2 = ts/(fs/(cstrat_prea*m_strat*1e-9*28*1e-12)) # fractionation of the sink for del
        return [res1,res2]
    ...
    if isinstance(d15strat_prea, float): # For some reason, this is nearly always a float in an array, but sometimes just a float...
        results_d15[0,0:3] = [d15_prea[0],d15strat_prea,0]
    else:
        results_d15[0,0:3] = [d15_prea[0],d15strat_prea[0],0]
    for n in range(1,len(t)) :
    ...
    if isinstance(SPstrat_prea, float): # For some reason, this is nearly always a float in an array, but sometimes just a float...
        results_SP[0,0:3] = [SP_prea[0],SPstrat_prea,0]
    else:
        results_SP[0,0:3] = [SP_prea[0],SPstrat_prea[0],0]
    for n in range(1,len(t)) :
