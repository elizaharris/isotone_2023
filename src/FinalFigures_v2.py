...
if not os.path.exists(parentdir+"/isotone_2025/figs/"+run_name):
    os.makedirs(parentdir+"/isotone_2025/figs/"+run_name)
if not os.path.exists(parentdir+"/isotone_2025/outputs/"+run_name):
    os.makedirs(parentdir+"/isotone_2025/outputs/"+run_name)
...
voigt_forpca["N2O_flux_ug_m2_day"] = np.log(voigt_forpca["N2O_flux_ug_m2_day"]) # Log transform N2O to better deal with peaks
...
voigt_forpca_norm = pd.DataFrame(preprocessing.MinMaxScaler().fit_transform(voigt_forpca))
voigt_forpca_norm.columns = voigt_forpca.columns
from sklearn.decomposition import PCA
from sklearn.preprocessing import StandardScaler
pca = PCA()
pca_results = pca.fit_transform(voigt_forpca_norm)
# Scree plot
...
plt.savefig("figs/"+run_name+"/voigt_pca_scree.png") 
plt.savefig("figs/"+run_name+"/voigt_pca_scree.pdf") 
plt.show()
# Correlation plot
loadings = pca.components_.T * np.sqrt(explained_variance)
pc1 = 0; pc2 = 7; # Select which PCs to show
...
plt.xlim(-0.35,0.35)
plt.ylim(-0.35,0.35)
plt.savefig("figs/"+run_name+"/voigt_pca_pc"+str(pc1+1)+"-"+str(pc2+1)+".pdf") 
plt.show()
