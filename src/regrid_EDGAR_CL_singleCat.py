...
for c in cats:
    ...
    for n in range(0,years.shape[0]) :
        print(years[n])
        if (c == "ENE") | (c=="CHE"):
            filename = "../isotone-rawdata/EDGAR/"+c+"_nc/v6.0_N2O_"+str(years[n])+"_"+c+".0.1x0.1.nc" 
        else: 
            filename = "../isotone-rawdata/EDGAR/"+c+"_nc/v50_N2O_"+str(years[n])+"_"+c+".0.1x0.1.nc"    
    ...
    exec(c + "_griddata = EDGAR_griddata")
    ...
    print("Extrapolation for",c,"of",cats)
    exec("EDGAR_griddata = "+ c + "_griddata")
    EDGAR_extrap = np.zeros((len(nyears),EDGAR_griddata.shape[1],EDGAR_griddata.shape[2]))+np.nan
    
    # Split the data into rows (or it crashes...)
    for r in np.arange(0,EDGAR_griddata.shape[1]):
        # Select the row
        print("Running row",r,"of",EDGAR_griddata.shape[1])
        block = EDGAR_griddata[:,r,:]
        
        # Identify the gridcells to extrapolate (non-0, non-nan)
        block_sum = np.nansum(block,axis=0)
        block_mask = block_sum/block_sum 
        nxa = np.where(block_mask==1)[0]
        block_toextrap = block[:,nxa]
        
        block_extrap = np.zeros((len(nyears),block_toextrap.shape[1]))+np.nan # Space for extraps
        for n in np.arange(0,block_toextrap.shape[1]) :
            if sum(~np.isnan(block_toextrap[:,n]))>2 :
                # Extrapolate early data using the first 15 years
                tmp = years <= min(years)+15
                m = LinearRegression(fit_intercept=False).fit(years[tmp].reshape(-1, 1)-startyear,block_toextrap[tmp,n])
                fit = (nyears-startyear)*m.coef_ + m.intercept_
                fit[fit<0] = 0
                block_extrap[:,n] = fit
                # Extrapolate current data using the last 15 years
                tmp = years >= max(years)-15
                m = LinearRegression().fit(years[tmp].reshape(-1, 1)-startyear,block_toextrap[tmp,n])
                fit = (nyears-startyear)*m.coef_ + m.intercept_
                fit[fit<0] = 0
                tmp = nyears >= max(years)-15
                block_extrap[tmp,n] = fit[tmp]
                # Use the real data where available
                a = np.where(nyears == years[0])[0][0]
                block_extrap[np.arange(a,(a+len(years))),n] = block_toextrap[:,n]
        # Return to full shape
        block_full = np.zeros((len(nyears),block.shape[1]))
        block_full[:,nxa] = block_extrap
        EDGAR_extrap[:,r,:] = block_full
    ...
    #%% save the cat data as a netcdf file
    ncout = nc4.Dataset('data/largeData/EDGAR_data_extrap_'+c+'.nc','w','NETCDF4'); # using netCDF4 for output format 
    ...
    catdata = ncout.createVariable('catdata','f4',('time','lat','lon'))
    catdata.setncattr('units','g N2O-N/m2/year')
    catdata.setncattr('source','EDGAR category: '+c)
    catdata.setncattr('category',c)
    catdata[:,:,:] = EDGAR_extrap[:,:,:]
    ncout.close()

    # test the file!
    f = nc4.Dataset('data/largeData/EDGAR_data_extrap_'+c+'.nc','r')
    n = 100
    plot_map(LON,LAT,f.variables['catdata'][n,:,:],"cat emissions (g N2O-N/m2/year): "+str(f.variables["time"][n]))

#%% save the full data as a single netcdf file
if 0:
    ncout = nc4.Dataset('data/largeData/EDGAR_data_extrap.nc','w','NETCDF4'); # using netCDF3 for output format 
    ncout.createDimension('lon',LON.shape[1])
    ncout.createDimension('lat',LAT.shape[0])
    ncout.createDimension('time',len(nyears))
    lonvar = ncout.createVariable('lon','f4',('lon'))
