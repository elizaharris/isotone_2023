...
# join the datasets
N2O_atmos = pd.concat([N2O_Park1,N2O_Park2,N2O_GAW,N2O_longfei,N2O_Sowers,N2O_Mach,N2O_CG,N2O_LawDome],join='outer',sort=False)
...
# plot the data
datasets = ("Longfei","Park_Archive","Park_Firn","GAW","Machida","Sowers_2002","CG_online","LawDome")
syms = ("bo","rx","ro","go","md","cx","yo","mx")
...
model = LinearRegression().fit(np.array(N2O_atmos_df["dt"].iloc[a]).reshape(-1, 1), N2O_atmos_df[column].iloc[a])
print(model.coef_)
...
N2O_final.to_csv('data/atmos_data/AllAtmosData.csv', index = False)
N2O_atmos_df.to_csv('data/atmos_data/SummAtmosData.csv', index = False)
