...
def IsoModel(d15Nsoil_vec,N_processes,fNH3_vec,fignum=np.nan,d15N_inat = -1.5,E_NH3 = -17.9,E_L	= [-1,0,5],E_nit = [-56.6,7.3],E_denit = [-31.3,6.1],E_denit_N2O = [-14.9,6.7],
    ...
    spin_up = 10 # time to reach steady state (arbitrary units, basically iterations)
    ...
    soil_NO3[0,1,:] = soil_NO3[0,0,:] # initial del of 0 permil so 15N = 14N (all is considered relative to int stand ratio)
    ...
    # Gaseous loss params
    E_G = E_nit[0]*N_processes[:,4]+E_denit[0]*N_processes[:,3] # net frac factor for gas losses (denit + nit)
    ...
    for its in range(0,4): # run 5 iterations; 3 are sufficient for almost all d15N values anyway...
        ...
        if its > 0: 
            k_G = k_G-(d15Nsoil_vec-soil_NO3_steady)/E_G # For all but the first iteration, correct towards steady state to find k_G
        k_G = k_G.clip(0,1)
        ...
        k15a_G_nN2O = k15_G_nN2O*(0.5*E_SP_nit[0]/1000+1) # 0.5 is an SP of 0; split according to actual SP
        ...
        k15a_G_dN2O = k15_G_dN2O*(0.5*E_SP_denit[0]/1000+1) # 0.5 is an SP of 0; split according to actual SP
        ...
        k15a_red = k15_red*(0.5*E_SP_red[0]/1000+1) # 0.5 is an SP of 0; split according to actual SP
        k15b_red = k15_red*(0.5*-E_SP_red[0]/1000+1)
        
        ### Run spin up to achieve steady state within this iteration
    ...
    # Make a final plot
    # fignum is the index of the gridcell to plot
    if ~np.isnan(fignum):
    ...
    # check simplex (sometimes a tiny bit off summing to 1)
    k_L = k_L/(k_L+k_NH3+k_G)
    k_NH3 = k_NH3/(k_L+k_NH3+k_G)
    k_G = k_G/(k_L+k_NH3+k_G)
    
    # Produce outputs: matrix with rows = d15N values and columns =
    # 0:5   k_L k_NH3 k_G soil_d15NNO3_final N2O_d15N N2O_SP
    # leave as matrix (no df) to save processing time
